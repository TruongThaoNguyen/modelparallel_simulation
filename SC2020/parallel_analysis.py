...
def core_analysis_filter(network, platform,layerconfig, g, metaData,nodeNumber,miniBatch,testName,results):
	...
	if memPerNode > g.MEM_PER_NODE:
		...
		for i in range(0,len(network['lays'])):
			layer = network['lays'][i]
			if layerconfig[i] == 1:
				Tcomm_layer = 2 * (nodeNumber-1)*(alpha + (miniBatch*float(layer['out'])*g.BYTE_PER_ITEM*beta)/nodeNumber)
				print i, float(layer['out'])*g.BYTE_PER_ITEM, Tcomm_layer,alpha,beta
				Tcomm1 = Tcomm1 + 3 * Tcomm_layer /2
		
		Tcomm1 = Tcomm1 * math.ceil(g.TOTAL_SAMPLE/miniBatch)
		print "TEST", Tcomm, Tcomm1
		print "ITER TIME:",nodeNumber, math.ceil(g.TOTAL_SAMPLE/miniBatch), Tcomp/math.ceil(g.TOTAL_SAMPLE/miniBatch), Tcomm*1/math.ceil(3*g.TOTAL_SAMPLE/miniBatch),Tcomm*2/math.ceil(3*g.TOTAL_SAMPLE/miniBatch)
	...
	for i in range(minIdx,maxIdx+1): #from 1 actual node
		...
		if g.FIX_MIRCO_BATCH is not None:
			...
			if (miniBatch <= g.MAX_MINIBATCH) and (nodeNumber >= P2):
				...
				alpha = latency1
				bandwidth1 = bandwidth1 / P2 #Because the connection from 1 GPU via PCIe links to GPU use the same link and also share with other flow. So that it reduce the bandwidth 4 times (the case of self contention)
				beta = 1/float(bandwidth1)
				#Communication the halo exchange. Only apply for CONV and POOLING (which has kernel size > 1)
				# A GPU only need to communicate with its preceding and next GPUs along the ring except the first GPU and the last GPU (near the border of sample). Communication will be performed in 2 round in 2 directions clockwise and counter-clockwise along the ring so that no network conflict appear!!! 
				# Secondly, we group the communication of B sample at the same time to reduce the latency. 
				...
				bandwidth1, latency1 = get_network_factor(platform,P2,ALGORITHM_RING, GPUDIRECT=False)
				alpha = latency1
				bandwidth1 = bandwidth1 / P2 #Because the connection from 1 GPU via PCIe links to GPU use the same link and also share with other flow. So that it reduce the bandwidth P2 times (the case of self contention)
				beta = 1/float(bandwidth1)
				...
				#Note: In ChainerMNx implementation, it implement allgatherv and alltoall in the backward...
				totalOutatSplit = float(network['lays'][splitLayerIdx]['out'])
				...
				print "LOCAL Allreduce",Tallreduce, alpha, beta
				
				#Perform the reducetion between node (leader)
				#(1 flows at the same time do not reduce the bandwidth 4 times)
				bandwidth2, latency2 = get_network_factor(platform,nodeNumber,ALGORITHM_RING)
				alpha = latency2
				bandwidth2 = bandwidth2 / 2 #Because the connection from leader to outside use the same PLX links. So that it reduce the bandwidth by 50% (the case of self contention)
				beta = 1/float(bandwidth2)
				...
				print "GLOBAL Allreduce",2*(P1-1)*(alpha + totalWeight*g.BYTE_PER_ITEM*beta/P1), alpha, beta
...
def get_network_factor(plat,rank, algorithm, GPUDIRECT=True):
	...
	if GPUDIRECT == False:
		if (rank <= 4):
			...
			CABLE_LENGTH = 5 #5 meter for intra-rack cable length
			latency = max(INTRA_SWITCH_LAT + 2* INTRA_CALBE_LAT, 4*(PLX_CABLE_LAT2+PLX_LAT) +  INTER_SWITCH_LAT_LEAF + 2*CALBE_LAT)
